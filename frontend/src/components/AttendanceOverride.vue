<template>
  <div class="space-y-6">
    <!-- Filters & Action Bar -->
    <div class="glass-panel p-4 sm:p-5 rounded-2xl border border-gray-800/80 space-y-4">
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div class="flex items-center gap-2 bg-gray-900/80 border border-gray-700/60 rounded-xl px-3 py-2 text-xs text-gray-200">
            ...
            <input 
              ...
              class="bg-transparent text-xs text-gray-100 outline-none cursor-pointer w-full"
            />
          </div>
        ...
        <!-- Filter Status Pills (Horizontal Touch Scrollable on Mobile) -->
        <div class="flex items-center gap-1.5 overflow-x-auto pb-1 lg:pb-0 touch-pan-x no-scrollbar">
          <button
            ...
            :class="[
              ...
              'px-3 py-1.5 rounded-lg text-xs border transition-all whitespace-nowrap min-h-[34px]'
            ]"
          >
            {{ st }}
          </button>
    ...
    <div class="glass-panel rounded-2xl border border-gray-800/80 overflow-hidden shadow-xl">
      ...
      <div class="px-4 sm:px-6 py-4 border-b border-gray-800/80 flex items-center justify-between">
        <div>
          <h2 class="text-sm sm:text-base font-semibold text-white flex items-center gap-2">
            <UserCheck class="w-4 h-4 sm:w-5 sm:h-5 text-indigo-400" />
            Daily Attendance Records
          </h2>
          <p class="text-[11px] sm:text-xs text-gray-400 mt-0.5">Showing records for {{ attendanceStore.selectedDate }}</p>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-xs text-gray-400 hidden sm:inline">Total Records:</span>
      ...
      <!-- Responsive Data Table / Touch Scroll -->
      <div class="overflow-x-auto touch-pan-x">
        <table class="w-full text-left text-xs text-gray-300 min-w-[640px]">
          <thead class="bg-gray-900/90 border-b border-gray-800 text-gray-400 uppercase text-[10px] tracking-wider">
            <tr>
              <th scope="col" class="px-4 sm:px-6 py-3.5">Employee</th>
              <th scope="col" class="px-4 sm:px-6 py-3.5">Department</th>
              <th scope="col" class="px-4 sm:px-6 py-3.5">Check-In Time</th>
              <th scope="col" class="px-4 sm:px-6 py-3.5">Status</th>
              <th scope="col" class="px-4 sm:px-6 py-3.5">Audit / Reason</th>
              <th scope="col" class="px-4 sm:px-6 py-3.5 text-right">Admin Actions</th>
            </tr>
          </thead>
            ...
            >
              ...
              <td class="px-4 sm:px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center font-bold text-white text-xs shadow-md shrink-0">
                    {{ getInitials(record.full_name) }}
                  </div>
                  <div>
                    <div class="font-semibold text-gray-100 text-xs sm:text-sm">{{ record.full_name }}</div>
              ...
              <td class="px-4 sm:px-6 py-4 max-w-xs">
                <div v-if="record.override_reason" class="text-xs space-y-0.5">
                  <div class="text-gray-300 line-clamp-1 text-[11px]" :title="record.override_reason">
              ...
              <td class="px-4 sm:px-6 py-4 text-right">
                <button
                  ...
                  class="px-3 py-1.5 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 border border-indigo-500/30 hover:border-indigo-400 transition-all font-medium text-xs flex items-center gap-1.5 ml-auto min-h-[34px]"
                >
