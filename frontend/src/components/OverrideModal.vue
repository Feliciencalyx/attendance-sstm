    ...
    <div class="glass-modal w-full max-w-lg rounded-2xl border border-gray-700/60 p-6 shadow-2xl relative">
      ...
      <div class="flex items-center justify-between border-b border-gray-800 pb-4 mb-5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-lg font-semibold text-white">Manual Status Override</h3>
            <p class="text-xs text-gray-400">Admin Audit Protocol Enforcement</p>
          </div>
        </div>
        <button 
          @click="close" 
      ...
      <div class="bg-gray-900/60 rounded-xl p-4 mb-5 border border-gray-800/80">
        <div class="grid grid-cols-2 gap-3 text-sm">
          <div>
            <span class="text-xs text-gray-500 block">Employee Name</span>
            <span class="font-medium text-gray-200">{{ record?.full_name }}</span>
          </div>
          <div>
            <span class="text-xs text-gray-500 block">Employee ID</span>
            <span class="font-mono text-gray-300">{{ record?.employee_id }}</span>
          </div>
          <div>
            <span class="text-xs text-gray-500 block">Current Date</span>
            <span class="text-gray-300">{{ record?.date }}</span>
          </div>
          <div>
            <span class="text-xs text-gray-500 block">Current Status</span>
            <span 
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-2">
            <button
              ...
              :class="[
                ...
                'px-3 py-2.5 rounded-xl text-xs border transition-all flex items-center justify-center gap-2'
              ]"
            >
              <CheckCircle v-if="targetStatus === s" class="w-4 h-4 text-indigo-400" />
              {{ s }}
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <input
            v-model="adminId"
            type="text"
            class="w-full bg-gray-950/80 border border-gray-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl px-3 py-2 text-xs font-mono text-gray-300 outline-none"
          />
        </div>
        ...
        <div class="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-[11px] text-amber-300/90 flex items-start gap-2">
          ...
          <span>
            <strong>Immutable Audit Policy:</strong> This override action will be permanently logged in the <code>audit_logs</code> table with your Admin ID, timestamp, prior status, and detailed justification.
          </span>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-800">
          ...
          <button
            ...
            class="px-5 py-2.5 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-600/30 transition-all flex items-center gap-2 disabled:opacity-50"
          >
